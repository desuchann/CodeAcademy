...
def null_heuristic(start, target):
    return 0

# do the biz
def a_star(graph, start, target, heuristic_func):
    paths_and_distances = {vertex: [inf, []] for vertex in graph}
    paths_and_distances[start] = [0, [start.name]]

    vertices_to_explore = [(heuristic_func(start, target), start)]

    while vertices_to_explore:
        _, current_vertex = heappop(vertices_to_explore)
        current_cost = paths_and_distances[current_vertex][0]
        ...
        for neighbor, edge_weight in graph[current_vertex]:
            ...
            if new_cost < paths_and_distances[neighbor][0]:
                paths_and_distances[neighbor][0] = new_cost
                paths_and_distances[neighbor][1] = paths_and_distances[current_vertex][1] + [neighbor.name]
                priority = new_cost + heuristic_func(neighbor, target)
                heappush(vertices_to_explore, (priority, neighbor))
